...
class Comentario{
	...
	public function excluirComentarios($id_comentario,$id_usuario){
		...
		if($id_usuario == 1){
			...
			$cmd = $this->pdo->prepare("DELETE FROM comentarios3 WHERE id = :ic");
			$cmd->bindValue(":ic", $id_comentario);
			$cmd->execute();

		}else{

			$cmd = $this->pdo->prepare("DELETE FROM comentarios3 WHERE id = :ic AND pk_id_usuario = :iu");
			$cmd->bindValue(":ic", $id_comentario);
	...
	{
		$cmd = $this->pdo->prepare("INSERT INTO comentarios3 (comentario,dia,horario,pk_id_usuario)
			VALUES (:c,:d,:h,:id_u)");
